...
from compemd.sinkhornOT import element_distance_matrix, ModifiedPettiforScale, emdloss
from data.dataset import FormulaDataset
from gans.general import F, criterion_GAN, discriminate
from gans.logger import Logger
from gans.networks import Discriminator, print_network, weights_init, ConditionalResnetGenerator, \
...
class AugCycleGan:
    ...
    def update_lr(self):
        ...
        for param_group in self.optimizer_G_B.param_groups:
            param_group['lr'] = self.lr / self.lr_divider
        print(('update learning rate: %f -> %f' % (self.old_lr, self.lr)))
    ...
    def train(
        ...
        print("training with batchsize:", batchsize)
        ...
        if load_pre_trained:
            try:
                ...
                print("*** model loaded from: {}".format(load_epoch))
            except:
                pass
        ...
        for epoch in range(starting_epoch, nepochs + starting_epoch):
            ...
            if epoch % save_freq == 0 and epoch > 0:
                print("save epoch at:", epoch)
                self.save_epoch(epoch)
                self.save()

    def predict_from_prior(
            self,
            test_set: FormulaDataset,
            n_trials=50,
            std=5,
            back_to_train=True,
            unique=True,
        ...
        # pair_check = []
        # print("real A:", real_A.shape)
        # print("real B:", real_B.shape)
        # for i in range(len(real_A)):
        #     nz_a = tuple([i[0] for i in torch.nonzero(real_A[i]).detach().tolist()])
        #     nz_b = tuple([i[0] for i in torch.nonzero(real_B[i]).detach().tolist()])
        #     ispair = nz_a == nz_b
        #     pair_check.append(ispair)
        # print(collections.Counter(pair_check))

        # print(real_A.shape)
        # print(real_B.shape)
        # unique removes the pairing between real_A[i] and real_B[i], that is, they can be of different chem sys
        if unique:
            # real_A = torch.tensor(np.unique(real_A.detach().cpu().numpy(), axis=0), device=DEVICE)
            # real_B = torch.tensor(np.unique(real_B.detach().cpu().numpy(), axis=0), device=DEVICE)
        ...
        # print(real_A_encoded.shape, real_B_encoded.shape)
        # print(fake_A_encoded.shape, fake_B_encoded.shape)
        return real_A_encoded, real_B_encoded, fake_A_encoded, fake_B_encoded

    def predict_from_opt(
            self,
            test_set: FormulaDataset,
            zlim=10.0,
            steps=2000,
