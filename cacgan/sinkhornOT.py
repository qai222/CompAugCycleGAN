...
from cacgan.utils import create_dir, DEVICE

"""
compute earth mover distance between composition vectors using gpu
environment:
    - visual studio 2019 version 16.11.10
    - cuda 11.3 (11.6 does NOT work)
    - pytorch 1.10.2

cuda source code for calculating Wasserstein distance was taken from
https://github.com/t-vi/pytorch-tvmisc/blob/master/wasserstein-distance/Pytorch_Wasserstein.ipynb
by Thomas Viehmann tv@mathinf.eu
"""
...
build_name = "wasserstein"
build_dir = torch.utils.cpp_extension.get_default_build_root()
build_dir = os.path.join(build_dir, build_name)
create_dir(build_dir)

# compile cuda source, load if already compiled
try:
    wasserstein_ext = torch.utils.cpp_extension._import_module_from_library(build_name, build_dir, True)
    logging.warning("wasserstein loaded at: {}".format(build_dir))
except ImportError:
    logging.warning("compiling wasserstein...")
    wasserstein_ext = torch.utils.cpp_extension.load_inline("wasserstein", cpp_sources="", cuda_sources=cuda_source,
                                                            extra_cuda_cflags=["--expt-relaxed-constexpr"],
                                                            build_directory=build_dir)
    logging.warning("compilation success at: {}".format(build_dir))
...
def chememd_distance_matrix(encoded, distmat, chunk=2000):
    ...
    logging.warning("the distance matrix should have # of elements: {}".format(len(encoded)**2))
    logging.warning("number of elements in a triangle: {}".format(len(ijs)))
    logging.warning("compute emd distance matrix, this may take a while...")
    ...
    for i in tqdm(range(len(encoded_is) // chunk + 1)):
        ...
        logging.warning("chunk {}: {} {}".format(i, ta.shape, emds.mean()))
        dists += emds.tolist()
...
def check_against_pyemd():
    """testing sinkhorn against pyemd"""
    ...
    for i in range(a.shape[0]):
        logging.warning("pyemd gives: {}".format(emd(a[i], b[i], d)))
    ...
    logging.warning("sinkhorn gives: {}".format(emd_dist))
