...
def baseline_identity(real_A_encoded, real_B_encoded, ) -> [float]:
    """
    return the identity baseline between fake B (which is just real A) and real B
    """
    bl_B2B = []
...
def gen_rand_composition_vector(real_B_encoded) -> np.ndarray:
    """
    randomly generate a 2d array, row-wise normalized
    """
    rand_es = []
    for i in range(real_B_encoded.shape[0]):
        rb_e = real_B_encoded[i]
...
def baseline_random(real_B_encoded, ) -> [float]:
    """
    calculate baseline between random B and real B
    """
    bl_B2B = []
    np.random.seed(SEED)
    for i in range(real_B_encoded.shape[0]):
...
def eval_for_mindist(real_B_encoded, fake_B_encoded, ):
    """
    comparing each row of real B against its nearest fake B row from the entire fake_B array
    here fake B is a 3d array
    """
    min_B2B = []
...
def eval_for_dist(real_B_encoded, fake_B_encoded, ) -> [float]:
    """
    comparing each real B row against its nearest fake B row from the fake_B array generated using the real B row
    here fake B is a 3d array
    """
...
def eval_for_ratio(real_B_encoded, fake_B_encoded, possible_elements, e1, e2):
    """
    compute the e1/e2 ratio in compositions
    """
