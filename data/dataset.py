...
from data.schema import AtmoStructureGroup, FormulaEncoder, pd
from settings import seed_rng, SEED, random
from utils import composition_elements, diff2formula
from utils import save_pkl, Composition, chunks, convert_mno


def feat_compositions(feater: ElementProperty, comps: [Composition]):
...
def variance_threshold_selector(data, threshold=1e-5):
    selector = VarianceThreshold(threshold)
    selector.fit(data)
    return data[data.columns[selector.get_support(indices=True)]]


def rm_nan_columns(a: np.ndarray, m="any"):
    if m == "any":
...
class FormulaDataset(Dataset):
    ...
    def __init__(self, gab: GroupAB, mo: bool = False, mno: bool = False):
        ...
        if mno:
            ...
            self.gab = GroupAB(ga, gb)
            print(self.gab.possible_elements)
...
if __name__ == '__main__':
    ...
    # from the 2 most popular amine groups
    ...
    save_pkl(dataset_ab, "../dataset/dataset_ab.pkl")

    # # and mo only composition
    # dataset_ab = FormulaDataset(gab, mo=True, mno=False)
    # save_pkl(dataset_ab, "../dataset/dataset_ab_mo.pkl")
    ...
    save_pkl(dd, "../dataset/dimset.pkl")

    # # or with feats
    # dd = DimDataset.from_atmogroup(master_group, feat=True, mocomp=False)
    # save_pkl(dd, "../dataset/dimset_feat.pkl")
